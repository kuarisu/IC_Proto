    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (i.screenPos.rg*0.5);
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
                float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
////// Lighting:
////// Emissive:
                float node_8430 = 3.0;
                float3 emissive = floor(((_node_2015.rgb+saturate((dot(lightDirection,i.normalDir)*dot(i.normalDir,viewReflectDirection))))+saturate((_node_1697.rgb*pow((1.0-max(0,dot(i.normalDir, viewDirection))),5.0)))) * node_8430) / (node_8430 - 1);
...
////// Lighting:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_8430 = 3.0;
                o.Emission = floor(((_node_2015.rgb+saturate((dot(lightDirection,i.normalDir)*dot(i.normalDir,viewReflectDirection))))+saturate((_node_1697.rgb*pow((1.0-max(0,dot(i.normalDir, viewDirection))),5.0)))) * node_8430) / (node_8430 - 1);
