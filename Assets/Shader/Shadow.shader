    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_2694 = (i.screenPos.rg*0.2);
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + node_2694;
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
                float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
////// Lighting:
////// Emissive:
                float node_705 = saturate((dot(lightDirection,i.normalDir)*dot(i.normalDir,viewReflectDirection)));
                float3 node_6339 = (_node_1697.rgb*pow((1.0-max(0,dot(i.normalDir, viewDirection))),2.0));
                float3 node_1885 = ((_node_2015.rgb+node_705)+saturate(node_6339));
                float node_8430 = 3.0;
                float3 node_9122 = floor(node_1885 * node_8430) / (node_8430 - 1);
                float3 emissive = node_9122;
